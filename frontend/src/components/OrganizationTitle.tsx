...
export const OrganizationTitle: React.FC<Props> = ({ className }: Props) => {
  ...
  return width < TWO_LINES_ORGANIZATION_TITLE_MAX_WIDTH ? (
    <h3 className={`page-title two-lines-title uppercase-title color-primary ${className}`}>
    ...
    <h3 className="page-title uppercase-title color-primary">{t("organization.name")}</h3>
  );
};
